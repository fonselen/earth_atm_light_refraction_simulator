...
typedef struct
{
	...
	double camada_h[25001];	// define a altura de cada camada atmosferica
	double n[25001]; 		// define indice de refracao de cada camada atmosferica
...
int main(int argc, char* argv[])
{
	...
	reta_xy L_ray1[10000]; 	// declaração do conjunto de direções que representara o raio de luz na atmosfera
	...
	T.raio = 6000;		// raio
	altM = 1;			// altitude máxima da atmosfera considerada
	T.atm_num = 25000;		// numero de camadas
	esp = altM / T.atm_num;	//calculo da espessura de cada camada
	Po1.x = 5;			// atribuindo ponto de Teste x
	Po1.y = T.raio - 0.0005;				// atribuindo ponto de Teste y
	Po2.x = 0;
	Po2.y = T.raio + 0.02;
	...
	nray1 = teste_2(L_ray1, &T, Po1, 3.999755 * pi / 4 /*4*pi/4*/, 14);
	...
	//point = 0;
	z = 0; // determina a distancia de visualizacao
	...
	printf("\n\n Ponto de interseccao: (%lf, %lf)", L_ray1[camaux].B.x, L_ray1[camaux].B.y);
	printf("\n\n\n %d", camaux);
	//getch();
...
int teste_2(reta_xy* L_ray, Terra* T, ponto_xy P, double ang, int luz_cor)
{
	...
	while (i <= 9999)
	{
		...
		*/
		...
		{
			printf("\n Reflexão total na camada : %d - iteracao: %d", camada, i);
			pos = posicao_vetor_raio(v, L_ray[i].A, (*T).C);
