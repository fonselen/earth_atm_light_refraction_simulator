...
{
	ponto_xy* Data;
	int n_points; // numero de pontos de cada raio
	ponto_xy StartingPoint; // posicao inicial de cada raio
	long double StartingAng; // angulo inicial
...
void teste_2(LightRay* L_ray, Terra* T, long double xM); //funcao que calcula os raios refratados na atmosfera
...
double FindTargetPointAngle(LightRay* L_ray, Terra* T, ponto_xy TargetP, long double StartingAng, long double EndingAng);
double FindHorizonAngle(LightRay* L_ray, Terra* T, long double angi, long double xM);
void FindHorizonObjectIntersectio(LightRay* L_ray, Terra* T, double range);
void DataShow(ponto_xy Obs, ponto_xy GeometricHorizonPoint, ponto_xy LowestVisiblePoint, long double Range, long double ObjectSize);
...
int main(int argc, char* argv[])
{
	...
	long double m;
	long double ObjectDistance, ObjectSize, AngularDistance, DeltaHeight;
	ponto_xy AuxiliaryVector; //vetor auxiliar para calculos de direcoes
	...
	//digitar parametros
	printf("Enter Object Distance (km):");
	scanf_s("%lf", &ObjectDistance);
	printf("Enter Object Size (m):");
	scanf_s("%lf", &ObjectSize);
	ObjectDistance = ObjectDistance - 8.743;
   ...
   // Coordenadas do observador
	...
	L_ray1.StartingPoint = obs;
	L_ray1.StartingAng = 0;

	for (i = 0; i < 21; i++)
	{
		L_ray[i].n_points = 1013;
		L_ray[i].StartingPoint = obs;
		L_ray[i].StartingAng = 0;
	}
	...
	T.raio = 6370;		// raio;
	altM = 0.5;			// altura maxima para as camadas. No caso, as camadas serao consideradas ate essa altura.
	T.atm_num = 500000;		// numero de camadas
	esp = altM / T.atm_num;	//calculo da espessura de cada camada
	/*****************************************************************/

	//Atribuindo as coordenas da camera
	Pcam.x = ObjectDistance;
	Pcam.y = 6370;
	z = 0;
	...
	// L_ray[20] representa o raio maix baixo visiel, ou seja, representa o horizonte refratado
	
	target.x = (6370 + ObjectSize / 1000) * sin(ObjectDistance/6370) + 5.0;

	L_ray[20].StartingAng = 0.00148;

	L_ray[20].StartingAng = FindHorizonAngle(&L_ray1, &T, L_ray[20].StartingAng, target.x); //encontra o raio de luz do horizonte aparente, ou seja, o raio de luz mais baixo possivel.

	L_ray1.StartingAng = L_ray[20].StartingAng;
	teste_2(&L_ray1, &T, target.x);
	FindHorizonObjectIntersectio(&L_ray1, &T, ObjectDistance);
	LightRay_ResolutionConverter(L_ray1.Data, L_ray[20].Data, L_ray1.TotalReflectionPosition, L_ray1.n_points, L_ray[20].n_points);

	Paux1 = L_ray[20].Data[0];
	Paux2 = L_ray[20].Data[1];
	m = (Paux2.y - Paux1.y) / (Paux2.x - Paux1.x);
	L_rayRefracted[20].Data[0] = Paux1;
	L_rayRefracted[20].Data[1].x = target.x;
	...
	//Ajustando as coordenadas  do alvo
	target = L_ray[20].Data[1012];

	DeltaHeight = ((ObjectSize / 1000 + T.raio) - sqrt(target.x * target.x + target.y * target.y)) / 9; //calculo das distancias entre os targets

	if (DeltaHeight <= 0)
	{
		printf("\n\n\n ******************************************************************************");
		printf("\n O objeto nao pode ser visualizado");
		printf("\n Altura minima de visualizacao para essa distancia: %lf m", 1000 * ( sqrt(target.x * target.x + target.y * target.y) - T.raio) );
		printf("\n ******************************************************************************");
		printf("\n\n\n");
		return 0;
	}

	// Calculo das direcoes dos targets
	AuxiliaryVector.x = DeltaHeight * sin(ObjectDistance / T.raio);
	AuxiliaryVector.y = DeltaHeight * cos(ObjectDistance / T.raio);
	...
	{
		//Raio n refletido
		L_ray[i].StartingAng = FindTargetPointAngle(&L_ray1, &T, target, 0.0758, L_ray[20].StartingAng); //Encontra o angulo do raio que atingira o alvo no ponto target

		// Calcula a trajetoria de cada raio na atmosfera e retorna o numero de raios calculados.
		L_ray1.StartingAng = L_ray[i].StartingAng;
		teste_2(&L_ray1, &T, target.x);
		LightRay_ResolutionConverter(L_ray1.Data, L_ray[i].Data, L_ray1.TotalReflectionPosition, L_ray1.n_points, L_ray[i].n_points); //Armazena os pontos importantes nos raios de menor resolucao
		...
		L_rayRefracted[i].Data[1].x = target.x;
		...
		//Raio Refletido
		L_ray[i + 10].StartingAng = FindTargetPointAngle(&L_ray1, &T, target, -0.02, L_ray[20].StartingAng);

		// Calcula a trajetoria de cada raio na atmosfera e retorna o numero de raios calculados.
		L_ray1.StartingAng = L_ray[i + 10].StartingAng;
		teste_2(&L_ray1, &T, target.x);
		LightRay_ResolutionConverter(L_ray1.Data, L_ray[i + 10].Data, L_ray1.TotalReflectionPosition, L_ray1.n_points, L_ray[i + 10].n_points);

		Paux1 = L_ray[i + 10].Data[0];
		Paux2 = L_ray[i + 10].Data[1];
		m = (Paux2.y - Paux1.y) / (Paux2.x - Paux1.x);
		L_rayRefracted[i + 10].Data[0] = Paux1;
		L_rayRefracted[i + 10].Data[1].x = target.x;
		...
		target.x = target.x + AuxiliaryVector.x;
		target.y = target.y + AuxiliaryVector.y;
	...
	DataShow(obs, TangentPoint, L_ray[20].Data[1012], ObjectDistance, ObjectSize);
	...
	while (k == 0)
	{
		...
		if (L_ray_view == 1)
		{
			for (i = 0; i < 21; i++)
			{
				viewport_LightRay_render(L_ray[i].Data, Pcam.x, Pcam.y, z, L_ray[i].n_points, L_ray[i].color);
			}
		}
...
void teste_2(LightRay* L_ray, Terra* T, long double xM)
{
	int i;
	...
	long double ang_i, ang_r, ang;	// angulos de incidencia e refracao
	...
	v.x = cosl((*L_ray).StartingAng);
	v.y = sinl((*L_ray).StartingAng);


	// Atribui o ponto inicial ao primeiro ponto do conjunto de retas do raio
	(*L_ray).Data[0] = (*L_ray).StartingPoint;
	...
	while (i < 8999999)
	{
		...
		*/
		...
		else //indica que haverá reflexão total
		{
			...
			if ((*L_ray).TotalReflectionPosition == 0) (*L_ray).TotalReflectionPosition = i;
...
double FindTargetPointAngle(LightRay* L_ray, Terra* T, ponto_xy TargetP, long double StartingAng, long double EndingAng)
{
	...
	(*L_ray).StartingAng = Ang_A;
	teste_2(L_ray, T, TargetP.x);

	Pa = (*L_ray).Data[ (*L_ray).n_points - 1 ];

	(*L_ray).StartingAng = Ang_B;
	teste_2(L_ray, T, TargetP.x);

	Pb = (*L_ray).Data[(*L_ray).n_points - 1];
	...
	while (((Pa.y - Pb.y) > 0.00000001 || (Pb.y - Pa.y) > 0.00000001) && ((Ang_A - Ang_B) > 0.000000000000001 || (Ang_B - Ang_A) > 0.000000000000001))
	{
		...
		(*L_ray).StartingAng = ang_aux;
		teste_2(L_ray, T, TargetP.x);

		Paux = (*L_ray).Data[(*L_ray).n_points - 1];
...
double FindHorizonAngle(LightRay* L_ray, Terra* T, long double angi, long double xM)
{
	...
	(*L_ray).TotalReflectionPosition = 0;
	...
	while (factor > (1.0 / (1000000000000.0)))
	{
		...
		if ((*L_ray).TotalReflectionPosition == 0)
		{
			while ((*L_ray).TotalReflectionPosition == 0)
			{
		...
		if ((*L_ray).TotalReflectionPosition != 0)
		{
			while ((*L_ray).TotalReflectionPosition != 0)
			{
				...
				(*L_ray).color[1] = 40;
				(*L_ray).color[2] = 240;
				(*L_ray).TotalReflectionPosition = 0; // Recebe zero para calcular o proximo valor.
				ang_aux = ang_aux + 0.0005 * factor;
				(*L_ray).StartingAng = ang_aux;
				teste_2(L_ray, T, xM);
			}
...
void FindHorizonObjectIntersectio(LightRay* L_ray, Terra* T, double range)
{
	int i, aux;
	ponto_xy Pa, Pb;
	long double AngP, AngObj, a, b, r;

	AngObj = pi/2 - (range / (*T).raio); //angulo que o objeto faz com o eixo x

	Pa = (*L_ray).Data[0];

	for (i = 1; i < (*L_ray).n_points; i++)
	{
		Pb = (*L_ray).Data[i];

		AngP = acos(Pb.x / sqrt(Pb.x * Pb.x + Pb.y * Pb.y)); //angulo que o vetor posicao do poto faz com o eixo x
...
void DataShow(ponto_xy Obs, ponto_xy GeometricHorizonPoint, ponto_xy LowestVisiblePoint, long double Range, long double ObjectSize)
{
	...
	dist = sqrt(Obs.x * Obs.x + Obs.y * Obs.y) - 6370.0;
	printf("\n Altitude do ponto de observação: %.2lf m", dist * 1000);

	// distancia ponto de tangencia
	ang = (Obs.x * GeometricHorizonPoint.x + Obs.y * GeometricHorizonPoint.y);
	ang = ang / (sqrt(Obs.x * Obs.x + Obs.y * Obs.y) * sqrt(GeometricHorizonPoint.x * GeometricHorizonPoint.x + GeometricHorizonPoint.y * GeometricHorizonPoint.y));
	ang = acos(ang);
	dist = ang * 6370.0;
	...
	//Distancia do objeto
	dist = Range;
	printf("\n Distancia do objeto ao ponto de observacao: %.3lf km", dist + 8.743);

	//Altura do objeto
	dist = ObjectSize;
	printf("\n Altura do objeto alvo: %.3lf m", dist);

	//Ocultacao
	Paux = LowestVisiblePoint;

	dist = sqrt(Paux.x * Paux.x + Paux.y * Paux.y) - 6370.0;
	printf("\n Altura da parte nao visivel (ocultacao): %.2lf m", dist * 1000);

	//parte visivel
	dist = ObjectSize - (dist * 1000);
	printf("\n Altura da parte visivel: %.2lf m", dist);
