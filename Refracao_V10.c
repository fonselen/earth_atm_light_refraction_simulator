...
typedef struct
{
	...
	long double camada_h[20001];	// define a altura de cada camada atmosferica
	long double n[20001]; 		// define indice de refracao de cada camada atmosferica
	int atm_num; 			//numero de camadas amosfericas
} Terra;

int teste_2(ponto_xy* L_ray, Terra* T, ponto_xy P, long double ang, long double xM, int* pos_ReflexaoTotal); //funcao que calcula os raios refratados na atmosfera
...
int main(int argc, char* argv[])
{
	...
	long double ang_0[10];		// angulos de direcao iniciais para os raios de luz
	long double xM[10], z;						// profundidade da camera
	...
	while (1) {
		...
		if (Pcam.x > xM[0]) Pcam.x = Pcam.x - 0.2;
	...
	while (1) {
		...
		if (Pcam.x < 35) Pcam.x = Pcam.x + 0.2;
		else Pcam.x = 35;
...
int teste_2(ponto_xy* L_ray, Terra* T, ponto_xy P, long double ang, long double xM, int* pos_ReflexaoTotal)
{
	int i;
